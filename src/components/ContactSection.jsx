...
export default function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send email with template variables that match EXACTLY
      await emailjs.send(
        serviceID,
        templateID,
        {
          ...
          title: "New Portfolio Message", // Template uses {{title}} in subject
          time: new Date().toLocaleString(), // Template uses {{time}}
    ...
    <section id="contact" className="py-12 overflow-x-hidden bg-gray-900">
      <div className="px-4 mx-auto sm:px-6 max-w-7xl">
        ...
        <div className="px-2 mt-8">
          <div className="max-w-2xl mx-auto p-8 border border-gray-800 rounded-xl bg-gray-950 group hover:scale-[1.02] hover:-translate-y-1 transition-all duration-500 min-h-[500px]">
            <h4 className="mb-6 text-xl font-semibold text-center transition-colors duration-300 group-hover:text-blue-300 sm:text-2xl">
              Send Me a Message
            </h4>
            
            {isSent && (
              <div className="p-4 mb-6 text-base text-green-400 border border-green-800 rounded-lg bg-green-900/30 sm:text-lg">
                Message sent successfully! I'll get back to you soon.
              </div>
            )}
            
            {error && (
              <div className="p-4 mb-6 text-base text-red-400 border border-red-800 rounded-lg bg-red-900/30 sm:text-lg">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="group/input">
                  ...
                  />
                  <div className="absolute bottom-0 left-0 w-0 h-1 transition-all duration-500 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 group-hover/input:w-full"></div>
                </div>
                <div className="group/input">
                  <input
                    type="email"
              ...
              >
                {isLoading ? (
                  <>
                    <div className="w-6 h-6 border-white rounded-full border-3 border-t-transparent animate-spin"></div>
                    Sending...
                  </>
                ) : (
                  <>
                    <Send size={22} className="transition-transform duration-300 group-hover:rotate-12" />
                    Send Message
                  </>
                )}
              </button>
            ...
            <p className="mt-6 text-base text-center text-gray-500 transition-colors duration-300 group-hover:text-gray-400 sm:text-lg">
