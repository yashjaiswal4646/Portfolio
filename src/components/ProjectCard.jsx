...
export default function ProjectCard({
  ...
  return (
    <div 
      ref={cardRef}
      className={`relative overflow-hidden transition-all duration-500 border border-gray-800 bg-gray-950 rounded-xl group w-[90%] mx-auto max-w-md ${animationClass} ${resetClass}`} // Added width classes
      style={{
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="mt-6 transition-all duration-500 transform group-hover:scale-105 group-hover:translate-y-0">
          ...
          <div className="flex flex-wrap gap-2">
            {technologies.map((tech, idx) => (
              <span
                key={idx}
                className="px-3 py-1 text-xs font-medium text-blue-300 transition-all duration-300 border rounded-full cursor-default bg-blue-900/30 border-blue-700/50 hover:bg-blue-800 hover:text-white hover:scale-110 hover:shadow-lg hover:shadow-blue-900/30"
              >
                {tech}
              </span>
            ))}
        ...
        {/* Action Buttons - FIXED CURSOR ISSUE */}
        <div className={`mt-8 transition-all duration-500 ${
          ...
          <a
            ...
            className="relative z-20 flex items-center justify-center w-full gap-2 px-4 py-3 text-sm font-medium transition-all duration-300 bg-blue-600 rounded-lg cursor-pointer hover:bg-blue-700 hover:scale-105 hover:shadow-xl hover:shadow-blue-900/20"
        ...
        <div className="absolute inset-0 hidden transition-all duration-500 pointer-events-none bg-gradient-to-br from-blue-900/5 via-transparent to-purple-900/5 rounded-xl md:group-hover:block"></div>
      </div>

      {/* Shine effect on hover - Desktop only */}
      <div className="absolute inset-0 hidden transition-all duration-700 pointer-events-none bg-gradient-to-r from-transparent via-white/5 to-transparent group-hover:translate-x-full md:block"></div>
    </div>
  );
}